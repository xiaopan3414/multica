...
这个定制版允许一台 Windows Desktop 为每个智能体绑定一个已有目录，不需要移动项目，也不会为每个
task 重新克隆仓库。打开 **智能体 → 设置 → 通用 → 这台电脑上的私有工作目录**，选择目录即可。

- 映射只写入这台电脑的 Desktop CLI profile，不创建工作区共享的项目资源。
- 只有被这台 Desktop Daemon 领取、且属于该智能体的 task 才使用此目录；其他机器不受影响。
- 智能体直接修改已有目录，不创建 task 专用 checkout，也不启动该 task 的仓库缓存同步。
- 指向同一个真实路径的 task 会依次运行，避免两个智能体同时改同一份文件。
- 小队 leader task 继续使用普通隔离工作区，不绑定该目录。
- 修改或清除绑定时，如果 Daemon 正在执行 task，会先要求等待 task 完成；空闲 Daemon 会自动重启并加载新配置。
