...
| `CreateAgentRequest` fields | `CreateAgentRequest` | Includes optional `owner_id` plus `model`, `thinking_level`, and Codex `service_tier` alongside the profile/runtime/permission inputs |
...
| Delegated create owner | `CreateAgent` owner-resolution block | Omitted/current `owner_id` keeps the requester as owner. A different owner is accepted only from a human workspace `owner`, must equal the selected `public` runtime's owner, and must resolve through `GetMemberByUserAndWorkspace`; admins, members, agent actors, private runtimes and mismatched/non-member targets are rejected before insert. |
...
| `CreateAgent` insert params | `agent.go` create path | Persists the resolved agent owner plus avatar_url, runtime_config, instructions, custom_env, custom_args, model, thinking_level, service_tier, mcp_config, visibility, max_concurrent_tasks. Invocation-target `created_by`, events, and analytics retain the actual requester rather than the delegated owner. |
...
| Contract | Line | Behavior |
|---|---|---|
| `CreateAgent` INSERT | generated from `queries/agent.sql` | columns include `owner_id, runtime_config, runtime_id, instructions, custom_env, custom_args, mcp_config, model, thinking_level, service_tier` |
| `CreateAgentParams` | generated from `queries/agent.sql` | typed params include `OwnerID` plus nullable `Model`, `ThinkingLevel`, and `ServiceTier` |
