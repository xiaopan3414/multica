...
type Task struct {
	...
	AgentWorkingDirectory         string                 `json:"agent_working_directory,omitempty"`          // server-synchronized private path for this agent on the claiming daemon
	AgentWorkingDirectoryResolved bool                   `json:"agent_working_directory_resolved,omitempty"` // true even when empty; prevents stale local config from overriding a server reset
